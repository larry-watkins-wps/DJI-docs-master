# `drc_night_lights_state_set` — enable / disable beacon (night-flight) lights

DRC command that turns the aircraft's beacon lights on or off.
...
|---|---|---|
| Cloud → Device | `thing/product/{gateway_sn}/drc/down` | `drc_night_lights_state_set` |
| Device → Cloud | `thing/product/{gateway_sn}/drc/up` | `drc_night_lights_state_set` |

## Down — `data` fields

| Field | Type | Description |
|---|---|---|
| `night_lights_state` | enum int | `0` = Disabled; `1` = Enabled. |
