# `battery_maintenance_switch` — enable / disable aircraft-battery maintenance mode

Cloud command that enables or disables the aircraft-battery maintenance cycle. The reply ACKs task state; no progress event is associated with this method.
...
|---|---|---|
| Cloud → Device | `thing/product/{gateway_sn}/services` | `battery_maintenance_switch` |
| Device → Cloud | `thing/product/{gateway_sn}/services_reply` | `battery_maintenance_switch` |
