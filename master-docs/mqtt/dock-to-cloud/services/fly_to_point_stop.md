# `fly_to_point_stop` — end flyto to target point

Cloud command that ends an in-progress flyto task (started by [`fly_to_point`](fly_to_point.md) or [`takeoff_to_point`](takeoff_to_point.md)). On stop, the aircraft hovers at its current position instead of continuing to the target.
...
|---|---|---|
| Cloud → Device | `thing/product/{gateway_sn}/services` | `fly_to_point_stop` |
| Device → Cloud | `thing/product/{gateway_sn}/services_reply` | `fly_to_point_stop` |
