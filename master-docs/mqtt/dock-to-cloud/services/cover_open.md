# `cover_open` — open the dock cover

Cloud command that opens the dock cover. `Data: null`. The reply ACKs task state; progress streams via the [`cover_open`](../events/cover_open.md) event.
...
|---|---|---|
| Cloud → Device | `thing/product/{gateway_sn}/services` | `cover_open` |
| Device → Cloud | `thing/product/{gateway_sn}/services_reply` | `cover_open` |
