# `speaker_play_stop` — stop PSDK speaker playback

Cloud command that stops whatever is currently playing on the PSDK speaker payload (TTS or pre-recorded audio) outside of an active DRC session. Sister method to the DRC-session variant [`drc_speaker_play_stop`](../drc/drc_speaker_play_stop.md).
...
|---|---|---|
| Cloud → Device | `thing/product/{gateway_sn}/services` | `speaker_play_stop` |
| Device → Cloud | `thing/product/{gateway_sn}/services_reply` | `speaker_play_stop` |
...
- DRC-session equivalent: [`drc_speaker_play_stop`](../drc/drc_speaker_play_stop.md).
- To replay the most recent audio after a stop, use [`speaker_replay`](speaker_replay.md).
