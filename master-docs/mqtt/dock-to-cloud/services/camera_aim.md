# `camera_aim` — double-tap-to-aim

Cloud command that drives the gimbal to center a specified pixel in the camera's field of view — equivalent to a double-tap gesture on the Pilot viewfinder.

Part of the Phase 4 MQTT catalog. Shared conventions live in [`../../README.md`](../../README.md).

**Cohort**: **Dock 2 + Dock 3** — identical payload.

---

## Topics

| Direction | Topic | Method |
|---|---|---|
| Cloud → Device | `thing/product/{gateway_sn}/services` | `camera_aim` |
| Device → Cloud | `thing/product/{gateway_sn}/services_reply` | `camera_aim` |

## Down — `data` fields

| Field | Type | Description |
|---|---|---|
| `payload_index` | string | Camera enumeration (`{type}-{subtype}-{gimbalindex}`). |
| `camera_type` | enum string | Lens. `ir` = infrared, `wide` = wide-angle, `zoom` = zoom. |
| `locked` | boolean | Heading-gimbal lock. `false` = only gimbal turns; `true` = aircraft heading locked with gimbal so both turn together. |
| `x` | double | Target pixel x, origin at lens upper-left, horizontal axis. Range `[0, 1]`. |
| `y` | double | Target pixel y, origin at lens upper-left, vertical axis. Range `[0, 1]`. |

### Example

```json
{
  "bid": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxx",
  "data": {
    "camera_type": "zoom",
    "locked": true,
    "payload_index": "39-0-7",
    "x": 0.5,
    "y": 0.5
  },
  "tid": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxx",
  "timestamp": 1654070968655,
  "method": "camera_aim"
}
```

## Up (reply) — `data` fields

| Field | Type | Description |
|---|---|---|
| `result` | integer | Return code. `0` = success; non-zero represents an error. |

### Example

```json
{
  "bid": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxx",
  "data": { "result": 0 },
  "tid": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxx",
  "timestamp": 1654070968655,
  "method": "camera_aim"
}
```

## Source provenance

| Source | Role |
|---|---|
| `[Cloud-API-Doc/docs/en/60.api-reference/20.dock-to-cloud/00.mqtt/20.dock/10.dock2/110.drc.md]` | v1.11 canonical (Dock 2). |
| `[DJI_Cloud/DJI_CloudAPI-Dock2-Live-Flight-Controls.txt]` | v1.15 (Dock 2). |
| `[DJI_Cloud/DJI_CloudAPI-Dock3-Live-Flight-Controls.txt]` | v1.15 (Dock 3) — identical to Dock 2 v1.15. |
