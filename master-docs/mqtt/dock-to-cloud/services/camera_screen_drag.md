# `camera_screen_drag` — screen-drag gimbal control

Cloud command that drives the gimbal in pitch/yaw by specifying angular speeds — analogous to touch-drag on the Pilot viewfinder.
...
|---|---|---|
| Cloud → Device | `thing/product/{gateway_sn}/services` | `camera_screen_drag` |
| Device → Cloud | `thing/product/{gateway_sn}/services_reply` | `camera_screen_drag` |
...
| `pitch_speed` | double (deg/s) | Gimbal pitch angular speed. |
| `yaw_speed` | double (deg/s) | Gimbal yaw angular speed. Effective only when the aircraft head is not locked (`locked=false`). |
