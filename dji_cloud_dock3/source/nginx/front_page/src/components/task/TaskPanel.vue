<template>
  ...
  <div class="plan-panel-wrapper">
    <a-table class="plan-table" :columns="columns" :data-source="plansData.data" row-key="job_id"
      :pagination="paginationProp" :scroll="{ x: '100%', y: 600 }" :loading="isLoading" @change="refreshData">
      <!-- 执行时间 -->
...
onMounted(() => {
  ...
  console.log('Loading task plans for workspace:', workspaceId)
  getPlans()
})
...
function getPlans () {
  ...
  getWaylineJobs(workspaceId, body).then(res => {
    console.log('getPlans API response:', res)
    if (res.code !== 0) {
      console.error('API error:', res.message)
      message.error('Failed to load task plans: ' + res.message)
      return
    }

    if (res.data && res.data.list) {
      console.log('Loading task plans:', res.data.list)
      plansData.data = res.data.list
      paginationProp.total = res.data.pagination.total
      paginationProp.current = res.data.pagination.page
      console.log('Current plansData:', plansData.data)
    } else {
      console.warn('No task plan data received')
      plansData.data = []
    }
  }).catch(error => {
    console.error('API call failed:', error)
    message.error('Failed to load task plans')
    plansData.data = []
  }).finally(() => {
...
function refreshData (page: Pagination) {
  console.log('Refreshing data with pagination:', page)
