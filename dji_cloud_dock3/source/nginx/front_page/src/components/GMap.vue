...
import {
  DeviceOsd, DeviceStatus, DockOsd,
  DroneInDockEnum,
  EDockModeCode,
  EGear, EModeCode, GatewayOsd,
  NetworkStateQualityEnum, NetworkStateTypeEnum, RainfallEnum
} from '/@/types/device'
...
export default defineComponent({
  ...
  setup () {
    ...
    const qualityStyle = computed(() => {
      console.info('deviceInfo.dock.basic_osd', deviceInfo)
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            const gcjCoords = wgs84togcj02(longitude, latitude)
            console.log('当前位置坐标:', { latitude, longitude, gcjCoords })
            // 使用当前位置初始化地图
            useGMapManageHook.globalPropertiesConfig(app, [gcjCoords[0], gcjCoords[1]])
          },
          (error) => {
            console.warn('无法获取当前位置，使用默认地图中心:', error)
            // 使用默认位置初始化地图
