...
import { Firmware, FirmwareQueryParam } from '/@/types/device-firmware'
...
// Smart camera switch - automatically handles stream stopping and starting
export const smartCameraSwitch = async function (body: {}): Promise<IWorkspaceResponse<any>> {
  const url = `${HTTP_PREFIX}/live/streams/smart-switch`
  const result = await request.post(url, body)
  return result.data
}
