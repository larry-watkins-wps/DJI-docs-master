<template>
  <div class="height-100">
    ...
    <div class="scrollbar height-100" :style="{ height: scorllHeight + 'px'}">
      <div id="data" class=" uranus-scrollbar" v-if="docksData.data && docksData.data.length > 0" @scroll="onScroll">
      ...
      <div v-else>
        <div v-if="isLoading" style="text-align: center; padding: 60px 0;">
          <a-spin size="large" />
          <div style="margin-top: 16px; color: hsla(0,0%,100%,0.65);">Loading dock devices...</div>
        </div>
        <a-empty v-else :image-style="{ height: '60px', marginTop: '60px' }" />
      </div>
    </div>
  </div>
</template>
...
onMounted(() => {
  ...
  // 检查workspaceId是否存在
  if (!workspaceId) {
    console.error('WorkspaceId is missing from localStorage')
    message.error('Workspace ID not found. Please login again.')
    return
  }

  console.log('Loading docks for workspace:', workspaceId)
  getDocks()
...
async function getDocks () {
  ...
  await getBindingDevices(workspaceId, body, EDeviceTypeName.Dock).then(res => {
    console.log('getDocks API response:', res)
    if (res.code !== 0) {
      console.error('API error:', res.message)
      message.error('Failed to load dock devices: ' + res.message)
      return
    }
    if (res.data && res.data.list) {
      console.log('Adding docks to data:', res.data.list)
      ...
      console.log('Current docksData:', docksData.data)
    } else {
      console.warn('No dock data received')
    }
  }).catch(error => {
    console.error('API call failed:', error)
    message.error('Failed to load dock devices')
  }).finally(() => {
