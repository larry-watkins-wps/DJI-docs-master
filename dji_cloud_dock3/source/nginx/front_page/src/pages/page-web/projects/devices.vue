...
import { h, onMounted, reactive, ref, UnwrapRef, onUnmounted } from 'vue'
...
function handleDockLiveStatus (payload: any) {
  ...
  console.log('收到机场直播状态更新:', payload.data)

  // 如果当前显示的是机场设备，则刷新设备列表
  if (judgeCurrentType(EDeviceTypeName.Dock)) {
    // 可以根据需要决定是否要刷新整个列表，或者只更新特定设备的状态
    getDevices(current.value[0], true)

    // 或者也可以只更新特定设备的状态，而不刷新整个列表
    // updateSpecificDeviceStatus(payload.data)

    message.info('机场直播状态已更新')
  }
}
