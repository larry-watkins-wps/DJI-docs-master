...
public abstract class AbstractLivestreamService {
    ...
    /**
     * Start livestreaming (generic version for custom output type)
     * @param gateway
     * @param request   data
     * @param typeRef   output type reference
     * @return  services_reply
     */
    public <T> TopicServicesResponse<ServicesReplyData<T>> liveStartPush(GatewayManager gateway, LiveStartPushRequest request, TypeReference<T> typeRef) {
        return servicesPublish.publish(
                typeRef,
                gateway.getGatewaySn(),
                LiveStreamMethodEnum.LIVE_START_PUSH.getMethod(),
