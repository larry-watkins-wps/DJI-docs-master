...
public class DeviceServiceImpl implements IDeviceService {
    ...
    public Boolean saveOrUpdateDevice(DeviceDTO device) {
        if (device.getDeviceName() == null || device.getNickname() == null) {
            if (device.getDeviceSn().startsWith("9N9CN")) {
                device.setNickname("RC PlUS 2");
                device.setDeviceName("RC PlUS 2");
            }
            else if (device.getDeviceSn().contains("8UUX")) {
                device.setNickname("DOCK 3");
                device.setDeviceName("DOCK 3");
            }
            else if (device.getDeviceSn().startsWith("1581F")) {
                device.setNickname("M4TD");
                device.setDeviceName("M4TD");
            }
            else {
                device.setNickname("Matrice4");
                device.setDeviceName("Matrice4");
            }
        }
    ...
    public Integer saveDevice(DeviceDTO device) {
        if (device.getDeviceName() == null || device.getNickname() == null) {
            ...
            else if (device.getDeviceSn().contains("m4td") || device.getDeviceSn().contains("1581F")) {
                device.setNickname("M4TD");
                device.setDeviceName("M4TD");
            }
            else {
