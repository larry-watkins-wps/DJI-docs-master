...
public class SDKDeviceService extends AbstractDeviceService {
    ...
    public void rcLiveStatusUpdate(TopicStateRequest<RcLiveStatus> request, MessageHeaders headers) {
        // Process RC live status update
        RcLiveStatus liveStatus = request.getData();
        if (liveStatus == null || liveStatus.getLiveStatus() == null) {
            return;
        }
        ...
        // Update live status for each camera
        for (RcLiveStatusData statusData : liveStatus.getLiveStatus()) {
            if (statusData.getVideoId() != null) {
                // Update the live status in the database or cache
                // This could involve updating a live status table or cache
                log.info("RC Live Status Update - Device: {}, VideoId: {}, Status: {}, Quality: {}", 
                    request.getFrom(), 
                    statusData.getVideoId(), 
                    statusData.getStatus(), 
                    statusData.getVideoQuality());
            }
        }

        // Push live status update to web clients if needed
        if (StringUtils.hasText(deviceOpt.get().getWorkspaceId())) {
            deviceService.pushOsdDataToWeb(deviceOpt.get().getWorkspaceId(), 
                BizCodeEnum.RC_LIVE_STATUS, 
                request.getFrom(), 
                liveStatus);
        }
    }
    ...
    public void dockLiveStatusUpdate(TopicStateRequest<DockLiveStatus> request, MessageHeaders headers) {
        // Process Dock live status update
        ...
        // Update live status for each camera
        for (DockLiveStatusData statusData : liveStatus.getLiveStatus()) {
            if (statusData.getVideoId() != null) {
                // Update the live status in the database or cache
                log.info("Dock Live Status Update - Device: {}, VideoId: {}, Status: {}, Quality: {}", 
                    request.getFrom(), 
                    statusData.getVideoId(), 
                    statusData.getStatus(), 
                    statusData.getVideoQuality());
